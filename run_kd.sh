...
do
for LR in  1e-4 #  1e-3 1e-4 #  1e-3 # 1e-5 # 5e-5 #  1e-4
do
for seed in 1234 2345 3456 # 1234 # 2345 3456 #4567 5678
do
for pattern_idx in  1 3 7 15 31 32 48 56 60 62 #62 61 59 55 47 31 # 1 2 4 8 16 32  #$(seq 1 31) --non_linear_tl 
do
CUDA_VISIBLE_DEVICES=$GPU python run_glue_nas.py --fp16 --kd_tl --only_kd_loss  --tl_kd_weight $tkw --entropy_threshold $th   --freeze_trained_models --switch_pattern_idx $pattern_idx --num_parts $NP  \
  ...
  --output_dir   results/fix-pattern${pattern_idx}-freeze-loaded_${BIG_MODEL}-distil${SMALL_MODEL}_${TASK_NAME}_epoch${EPOCH}_LR${LR}_BSZ${BSZ}_LEN${MAX_LEN}_seed${seed}_num_parts${NP}_kdtl_${tkw}_only_kd_loss \
  --seed $seed
done
done
done 
