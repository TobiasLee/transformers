...
from transformers.modeling_multiple_bert import BertForSequenceClassification, LinearReplacementScheduler, \
...
class ModelArguments:
    ...
    # param for selecting sub models
    small_model_index: Optional[int] = field(
        default=0, metadata={"help": "small model index, [6, 4, 3, 2, 1]"}
    )
...
def main():
    ...
    scc_layer_n_list = model.bert.encoder.small_layer_num  # [6, 4, 3, 2, 1]
    if training_args.do_train:  # do_train, copy the first layer
        model.bert.encoder.scc_layer = nn.ModuleList(
            [
                nn.ModuleList(
                    [deepcopy(model.bert.encoder.layer[ix] for ix in range(k))]
                ) for k in scc_layer_n_list
            ]
        )
    ...
    optimizer_grouped_parameters.extend([
        {'params': [p for n, p in model.bert.encoder.scc_layers.named_parameters() if
    ...
    if training_args.do_train:
        for i in range(len(scc_layer_n_list)):
            model.set_small_number_idx(i)  # set i
            replacing_rate_scheduler.reset()
