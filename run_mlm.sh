...
do
CUDA_VISIBLE_DEVICES=$GPU python run_lm_switch.py --fp16 --mlm  --tl_kd_weight $tkw --entropy_threshold $th  --switch_pattern_idx $pattern_idx --num_parts $NP  \
  ...
  --output_dir   results/fix-pattern${pattern_idx}-_${BIG_MODEL}-distil${SMALL_MODEL}_${TASK_NAME}_epoch${EPOCH}_LR${LR}_BSZ${BSZ}_seed${seed}_num_parts${NP} \
  --seed $seed
done
done
done 
