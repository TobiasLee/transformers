    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None,
        ...
        def _run_sub_blocks(layer_hidden_states, layers, idx):
            for i, layer in enumerate(layers):
                # print('layer :%d' %i )
                layer_output = layer(hidden_states, attention_mask[idx], None, encoder_hidden_states,
                                     encoder_attention_mask[idx])
        ...
        if self.training and not self.switch_mode:
        ...
        elif self.training and self.switch_mode:
            ...
            for i in range(self.num_parts):
                ...
                base_hiddens, base_outputs = _run_sub_blocks(base_input, self.scc_layer[i:i + base_interval], base_idx)
                large_hiddens, large_outputs = _run_sub_blocks(large_input, self.layer[i:i + large_interval], large_idx)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        if labels is not None:
            ...
            if action_probs is not None:
                bsz = logits.size()[0]
                final_decision_prob = torch.ones((bsz,))
                for path_prob in action_probs:
                    final_decision_prob *= torch.max(path_prob, dim=-1)  # final prob
                entropy_reward = CrossEntropyLoss(reduction='none')
                class_reward = torch.sum(entropy_reward * final_decision_prob)  # sum over bsz
