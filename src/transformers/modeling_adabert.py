...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, mlp_mask=None):
...
class BertClassificationHead(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        out_proj = self.classifier(pooled_output)
        return out_proj
...
class AdaBertEncoder(nn.Module):
    def __init__(self, config, original_encoder_layer, small_layer_num):
        ...
        self.shallow_layers = nn.ModuleList([
            nn.ModuleList([BertLayer(config) for _ in range(num)])
            for num in small_layer_num
        ])
    ...
    def forward(self,
        ...
        def _run_encoder_layer_forward(encoder_layers):
            ...
            for i, layer_module in enumerate(encoder_layers):
                inner_outputs = layer_module(inner_hidden, attention_mask, head_mask[i], encoder_hidden_states,
                                             encoder_attention_mask, mlp_mask[i])
        ...
        all_hiddens.extend(
            [_run_encoder_layer_forward(shallow_layer)
             for shallow_layer in self.shallow_layers])
...
class BertModel(BertPreTrainedModel):
    ...
    Examples::
        ...
        encoder_outputs = self.multiple_encoder(
            embedding_output,
        ...
        return encoder_outputs  # logits, (hidden_states), (attentions)
...
class MultipleBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
