...
export GPU="0,1" # ,1,2,3" # ,1,2,3" #,1,2,3"
EXIT_EPOCH=10.0
AGENT_EPOCH=1500.0
BSZ=64
EXIT_LR=2e-5 
...
for pr in 0.02 #0.11 0.115 0.12  #0.105  #0.001 0.01 0.1 #0.5 1.0 10.0 #0.1 0.01 0.001  #0.0002 0.0003 0.0005 0.0007 #0.1 0.01 0.001 # 5 #  0.0002 # 0.001 0.0001  #0.0002 #0.0005 0.0007  # 0.0005 0.0009 #  0.001 # 0.01 0.1 1.0 #  0.005 0.01 0.05 # 0.1  0.2 0.5  2.0 5.0 10.0  # 10.0 50.0 100.0 1000.0
do
SECOND_STAGE_OUTPUT=${OUTPUT_PATH}_nas_penalty_ratio${pr}_epoch${AGENT_EPOCH}_agent_lr${AGENT_LR}_fix_scc_early_exit_second_stage_action3/
CUDA_VISIBLE_DEVICES=$GPU python ./run_glue_theseus.py --logging_paths  --logging_steps 500  \
  --model_name_or_path  "$FIRST_STAGE_OUTPUT"  \
  --task_name $TASK_NAME --seed $SEED  --switch_mode --train_agent --fix_scc_layer --early_exit  \
  --do_eval  --do_train --fp16 \
  --data_dir "$GLUE_DIR/$TASK_NAME" \
  --max_seq_length 128 \
  --per_device_train_batch_size $BSZ \
